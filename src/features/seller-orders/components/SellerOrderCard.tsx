    ...
    <div className="bg-white rounded-3xl p-6 border border-stone-100 shadow-sm mb-4 transition-all hover:shadow-md">
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 flex flex-col pt-1">
          <h3 className="font-bold text-stone-800 line-clamp-2 md:w-3/4">{order.firstItemName}</h3>
          {order.itemCount > 1 && (
          ...
          <div className="mt-auto flex justify-between items-end">
            <div>
              <p className="text-xs font-bold px-2 py-1 bg-stone-100 text-stone-600 rounded-lg uppercase inline-block">
                {order.paymentStatus === 'PAID' ? 'Đã thanh toán (Ví)' : 'Thanh toán COD'}
              </p>
            </div>
            <div className="text-right flex items-center gap-6">
              <div>
                <p className="text-xs text-stone-500 font-medium">
                  Tổng tiền ({order.itemCount} sản phẩm)
                </p>
      ...
      <div className="mt-4 sm:hidden flex justify-end border-t border-stone-50 pt-4">
        <Link href={`/dashboard/cua-hang/don-hang/${order.orderCode}`} className="w-full">

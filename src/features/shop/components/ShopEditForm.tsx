    ...
    >
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="font-bold text-stone-800 text-sm">Thông tin pháp lý</h3>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Mã số thuế" error={errors.taxCode?.message} required>
            <input
              {...register('taxCode')}
              className={cn(inputCls, errors.taxCode && 'border-red-400')}
            />
          </Field>
          <Field label="Số ĐKKD" error={errors.businessRegNo?.message} required>
            <input
